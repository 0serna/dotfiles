...
- **AND** the replacement stub would reduce the estimated token count
- **THEN** DCP SHALL write the original error content to a temporary per-session file
...
- **AND** the replacement stub would reduce the estimated token count
- **THEN** DCP SHALL write the original older result content to a temporary per-session file
