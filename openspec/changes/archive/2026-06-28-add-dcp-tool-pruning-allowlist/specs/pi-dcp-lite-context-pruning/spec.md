...
#### Scenario: Same-tool superseded file result is stubbed for allowlisted tool

- **WHEN** a file `toolResult` targets a file that is targeted by a later operation with the same normalized tool and same normalized target
