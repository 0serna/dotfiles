...
The current `stale_large` rule uses a 1500-token threshold after the global window. Because this rule relies only on age and size, it needs a more explicit and conservative gate than semantic rules such as duplicate output or resolved errors.
...
- Add a dedicated age gate for `stale_large` using DCP-ageable tool results.
- Make size-based pruning more conservative with a 2500-token threshold.
- Reduce false positives in `superseded` by requiring the same normalized tool and target.
...
2. **Remove global recent-message protection.**
   - Decision: `duplicate`, `resolved`, and `superseded` can apply immediately when their deterministic conditions are met.
...
3. **Gate only `stale_large` by age and size.**
   - Decision: `stale_large` applies only when the candidate is older than 20 DCP-ageable tool results and exceeds 2500 estimated tokens.
...
4. **Keep `SKILL.md` protected from size-only pruning.**
   - Decision: Explicit `read` results for paths ending in `SKILL.md` remain excluded from `stale_large`, but can still be pruned by semantic rules.
...
6. **Rename protection metrics around the size gate.**
   - Decision: Replace `protectedRecentCount` with an `staleLargeProtectedCount`-style metric.
...
- Higher size threshold reduces token savings from `stale_large` → Mitigated by immediate semantic pruning and clearer safety.
