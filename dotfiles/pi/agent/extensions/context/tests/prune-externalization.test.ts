...
describe("context DCP pruning externalization", () => {
  ...
  it("omits saved= field when full-output path is missing", () => {
    ...
    const stubText = textOf(pruned[1]!);
    expect(stubText).toContain("reason=stale_large");
    expect(stubText).not.toContain("saved=");
    expect(stubText).not.toContain(missingPath);
  });

  it("omits saved= field when web_fetch full-content path is missing", () => {
  ...
  it("omits saved= field when no full-output path is present", () => {
    const messages = [
      assistantToolCall("a", "bash", { command: "rg foo" }),
